...
def getspec1Dname(mask, row, id, name):
   path = '{}_quick_spec1D'.format(mask)
   
   return '{}/{}_{}_{}_quick_spec1D.fits'.format(path, row, id, name)
...
#for CarPy output
def createmusefiles(mask, ra, dec, radius, cube):
   ...
   ra = np.array([ra])
   dec = np.array([dec])
   ra_string, dec_string, name = coord.coordstring(ra, dec)
   name = name[0].replace("[", "")
   name = name.replace("]", "")
   name = name.replace("'", "")
   print('name')
   ...
   objects = Table([row, id, [name], ra, dec, [radius]], names=('row', 'id', 'name', 'ra', 'dec', 'radius'))
   print(objects)
   ...
   # Crate object file
   objects.write('{}_quick_spec1D/{}_objects.fits'.format(mask, mask), overwrite=True)
...
class muse_redshiftgui:
   ...
   def __init__(self, mask, cube, xsize=1000, ysize=1000):
      ...
      self.cube = cube#mpdaf.obj.Cube('{}.fits'.format(self.mask))
...
#      self.param.children()[8].sigValueChanged.connect(self.setExtFlag)
   ...
   def save(self):
      ...
      self.setTable()
# 
      path = '{}_quick_spec1D'.format(self.mask)
...
parser.add_argument('-ra', metavar='RA [deg] of target for extraction', type=float, help='RA of target', required=False, default=-1.0)
parser.add_argument('-dec', metavar='Dec [deg] of target for extraction', type=float, help='DEC of target', required=False, default=-1.0)
parser.add_argument('-radius', metavar='radius [arcsec] for extraction', type=float, help='radius for extraction in arcsec', required=False, default=0.8)
parser.add_argument('-ra_string', metavar='RA [hh:mm:ss.] of target for extraction', type=str, help='RA of target', required=False, default='')
parser.add_argument('-dec_string', metavar='Dec [dd:mm:ss.] of target for extraction', type=str, help='DEC of target', required=False, default='')
parser.add_argument('-x', metavar='x as defined by MPDAF (index of first pixel is 0) unless otherwise specified', type=float, help='x of target with MPDAF/python/IDL indexing unless otherwise specified', required=False, default=-1.0)
parser.add_argument('-y', metavar='y as defined by MPDAF (index of first pixel is 0) unless otherwise specified', type=float, help='y of target with MPDAF/python/IDL indexing unless otherwise specified', required=False, default=-1.0)
parser.add_argument('-fitsindex', metavar='Boolean, True if fits indexing instead of python, false otherwise', type=bool, help='True if fits indexing, false if python indexing', required=False, default=False)
...
# Read in the cube   
cube = mpdaf.obj.Cube('{}.fits'.format(args.m))
wcs = cube.wcs
...
if args.ra_string != '':
   ...
   ra = coord.raparse(args.ra_string)
   dec = coord.decparse(args.dec_string)
   

if args.x > -1:
   deltaIndex = 0
   if args.fitsindex:
      print('Subtracting 1 from x, y index to convert to python instead of fits standard')
      deltaIndex = 1
   decra = wcs.pix2sky((args.y - deltaIndex, args.x - deltaIndex))[0]
   dec = decra[0]
   ra = decra[1]
...
if not os.path.isdir('{}_quick_spec1D'.format(args.m)):
   
   path = '{}_quick_spec1D'.format(args.m)
   os.mkdir(path)
   
   
print('Creating 1D spectrum files')
createmusefiles(args.m, ra, dec, args.radius, cube)
#     
#else:
#   print('1D spectrum files already present. Copying objects file to backup')
#   shutil.copy('{}_quick_spec1D/{}_objects.fits'.format(args.m,args.m),'{}_quick_spec1D/{}_objects_bkp.fits'.format(args.m,args.m))
#      
redshiftgui = muse_redshiftgui(args.m, cube, args.xsize, args.ysize)