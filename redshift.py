...
def fitatz_star(spec, z):
 ...
 # Same as fitatz_galaxy but for quasars with Hewitt and Wild cross-correlation template at flux calibration polynomials
def fitatz_qso_hw_poly(spec, z):
      ...
      return (eigenvalues, model, model_qso, model_poly, chi2)
...
def findz_qso_hw(spec, zmin=0.0, zmax=3.0, dz=0.0005):
   ...
   indices = np.arange(0, len(zs), dtype=int)
   redshifts = np.ndarray(len(zs),
                         dtype={'names':('index', 'z', 'chi2', 'eigen1', 'eigen2',
                                'fluxcal0', 'fluxcal1', 'fluxcal2'), 
   ...
   for redshift in redshifts:
      ...
      eigenvalues, chi2 = fitatz_qso_hw_precompute(redshift['z'])
      redshift['chi2'] = chi2
      redshift['eigen1'] = eigenvalues[0]
      redshift['eigen2'] = eigenvalues[1]
...
def fitz_galaxy(spec, zguess, fluxpoly=True):
   ...
   result = galaxy_model.fit(spec['flux'], wave=spec['wave'], weights=1/spec['error']**2,
...
def fitz_star(spec, zguess, fluxpoly=True):
   ...
   result = star_model.fit(spec['flux'], wave=spec['wave'], weights=1/spec['error']**2,
                             params=parameters, missing='drop')
   return result
