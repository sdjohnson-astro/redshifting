...
def getspec2Dname(mask, row, id):
   path = '{}_spec1D'.format(mask)
   
   return '{}/{}_{}_{}_spec2D.fits'.format(path, mask, row, id)
...
# 
   ...
   def setSpec(self, autoRange=True):
      ...
      self.flux2D = fits.getdata(getspec2Dname(self.mask, self.objects[self.row-1]['row'], self.id))
      ...
      dimen = self.flux2D.shape
      
      self.plot_spec2D_hist.setLevels(*np.percentile(self.flux2D[100:dimen[0]-100, 5:dimen[1]-5],[2.5,97.5]))
